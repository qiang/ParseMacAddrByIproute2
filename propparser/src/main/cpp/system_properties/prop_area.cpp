...
prop_area *prop_area::map_fd_ro(const int fd) {
    ...
    if (
//            (fd_stat.st_uid != 0) || (fd_stat.st_gid != 0) ||
...
prop_area *prop_area::map_prop_mem(void *memaddr, size_t size) {
    pa_size_ = size;
    pa_data_size_ = pa_size_ - sizeof(prop_area);

    prop_area *pa = reinterpret_cast<prop_area *>(memaddr);
...
prop_info *prop_area::new_prop_info(const char *name, uint32_t namelen, const char *value,
    ...
    if (valuelen >= PROP_VALUE_MAX) {
        ...
        char *long_location = reinterpret_cast<char *>(allocate_obj(valuelen + 1,
                                                                    &long_value_offset));
