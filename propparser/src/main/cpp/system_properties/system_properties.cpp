...
//https://cs.android.com/android/platform/superproject/+/master:bionic/libc/bionic/system_property_api.cpp;drc=b481a2e743102efc6b18cc586aa979a70e575d64;l=46
...
const prop_info *SystemProperties::Find(const char *name) {
    ...
    if (!pa) {
        async_safe_format_log(ANDROID_LOG_WARN, "my libc", "Access denied finding property \"%s\"",
                              name);
        return nullptr;
    }
...
//        async_safe_format_log(ANDROID_LOG_ERROR, "libc", "Could not find area for \"%s\"", pi->name);
    ...
    atomic_thread_fence(memory_order_release);
    atomic_store_explicit(&pi->serial, (len << 24) | ((serial + 1) & 0xffffff),
...
bool SystemProperties::Wait(const prop_info *pi, uint32_t old_serial, uint32_t *new_serial_ptr,
    ...
    do {
        int rc;
        if ((rc = __futex_wait(serial_ptr, old_serial, relative_timeout)) != 0 &&
            rc == -ETIMEDOUT) {
            return false;
        }
